...
{
    /// <summary>
    /// Helper class to access three-dimensional Fortran arrays.
    /// </summary>
    internal class F3DArrayInt32Access : FArrayAccess<Int32>
    {
        ...
        /// <summary>
        /// Create a new three-dimensional integer array accessor.
        /// </summary>
        /// <param name="ptr">Address of allocated array memory.</param>
        /// <param name="size">Sizes per dimension.</param>
        public F3DArrayInt32Access(IntPtr ptr, Int32[] size)
        {
            this.ptr = ptr;
            this.size = size;
        }
        ...
        {
            return F3DArrayInt32Access.GetArrayItem(this.ptr, this.size, index);
        }

        /// <see cref="FArrayAccess{T}.CopyTo(T[])" />
        public void CopyTo(Int32[] dest)
        {
            Int32 size = Math.Min(this.size[0] * this.size[1] * this.size[2], dest.Length);
            for (Int32 i = 0; i < this.size[0]; i++)
            {
                for (Int32 j = 0; j < this.size[1]; j++)
                {
                    for (Int32 k = 0; k < this.size[2]; k++)
                    {
                        Int32 index = (i * this.size[1] + j) * this.size[2] + k;
                        ...
                        dest[index] = this.GetItem(new Int32[] { i, j, k });
                    }
                }
            }
        }
        ...
        {
            F3DArrayInt32Access.SetArrayItem(this.ptr, this.size, index, value);
        }
            ...
            for (Int32 i = 0; i < this.size[0]; i++)
            {
                for (Int32 j = 0; j < this.size[1]; j++)
                {
                    for (Int32 k = 0; k < this.size[2]; k++)
                    {
                        ...
                        this.SetItem(new Int32[] { i, j, k }, source[index]);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Allocate new memory for a three-dimensional array.
        /// </summary>
        /// <param name="size">The sizes for the three dimensions.</param>
        ...
        /// <param name="size">Sizes of the three dimensions.</param>
        ...
        /// <param name="size">Sizes of the array for each of the three dimensions.</param>
        /// <param name="index">The three indices identifying the item.</param>
    ...
    public class F3DArrayFieldInt32 : FArrayField<Int32>
    {
        // Address of the owner of this field.
        protected IntPtr owner;

        // Cache to access manager.
        private F3DArrayInt32Access access;

        /// <summary>
        /// Access a three-dimensional integer array stored in a derived type.
        /// </summary>
        /// <param name="owner"></param>
        /// <param name="size"></param>
        /// <param name="getter"></param>
        public F3DArrayFieldInt32(IntPtr owner, Int32[] size, FTypeGetArray getter) : base(getter(owner), size)
        {
            this.owner = owner;
            this.access = new F3DArrayInt32Access(this.ptr, size);
        }
    ...
    /// </summary>
    public class F3DArrayInt32 : FStaticArray<Int32>
    {
        ...
        public F3DArrayInt32(Int32[] size) : base(size)
        {
            if (size.Length != 2)
            {
        ...
        public F3DArrayInt32(Int32 i, Int32 j) : base(new Int32[] { i, j })
        {
        }
        ...
        {
            this.access = null;
            return F3DArrayInt32Access.AllocateArray(size);
        }
        ...
        {
            this.access = null;
            F3DArrayInt32Access.FreeArray(this.ptr, this.size);
        }
