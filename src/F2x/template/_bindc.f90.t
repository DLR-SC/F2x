...
{%- macro var_decl(var, name, value=False, arg_names={}, intent=None) -%}
		...
		{%- if value and var.intent == 'IN' %}, VALUE{% endif -%}
		{%- if var.intent or intent %}, INTENT({{ var.intent or intent }})
		{%- endif %} :: {{ name }}
...
{%- macro var_assign(var, name, value) -%}
		...
		{%- elif var.strlen -%}
		CALL F_C_STRING({{ value }}, {{ var.strlen }}, {{ name }})
		{%- elif var.ftype -%}
		{{ name }}_INTERN => {{ value }}
...
{%- elif function.ret.getter == 'subroutine' %}
	...
	{%- endfor %}
		{{ var_decl(function.ret, export_name.upper() + '_VALUE', intent='OUT', value=True) }}
	{%- for arg in function.args %}
	{%- if arg.strlen %}
		CALL C_F_STRING({{ arg.name }}, {{ arg.strlen }}, {{ arg.name }}_INTERN)
	{%- elif arg.ftype %}
